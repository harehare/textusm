...
context('TextUSM', () => {
  ...
  it('Save the diagram to local and load it', () => {
    ...
    cy.get('[data-test="header-title"]').click().get('[data-test="header-input-title"]').type('test');
    // eslint-disable-next-line cypress/no-unnecessary-waiting
    ...
    cy.get('[data-test="diagram-list"]').find('[data-test="diagram-list-item"]').should('be.visible').first().click();
    cy.get('[data-test="editor"]').should('have.attr', 'value', 'test1\n    test2\n    test3');
  });

  it('Change background color of user story map', () => {
    cy.get('[data-test="new-menu"]').first().click().get('[data-test="new-usm"]').first().click();
    cy.get('[data-test="diagram"]').find('text').next().should('contain', 'USER ACTIVITY');
